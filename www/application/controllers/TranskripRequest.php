...
class TranskripRequest extends CI_Controller {
    ...
    public function add() {
        try {
            ...
            //CSRF
            //ganti code di config.php ( $config['csrf_protection'] = false;)


            //script Injection
            //input => <script>window.location.href = "https://www.youtube.com/watch?v=ZZ5LpwO-An4"</script>
            //ganti code diatas dengan code dibawah
            $this->db->insert('Transkrip', array(
                'requestByEmail' => $userInfo['email'],
                'requestDateTime' => strftime('%Y-%m-%d %H:%M:%S'),
                'requestType' => $requestType,
            ...
            // nambahin buat SQL injection
            //delete code Original, ganti dengan yang dibawah
            // $requestUsage = ($this->input->post('requestUsage'));
            // $requestDateTime = strftime('%Y-%m-%d %H:%M:%S');
            // $requestByEmail = $userInfo['email'];

            //inputan SQL injection
